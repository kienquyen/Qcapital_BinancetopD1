...
#Configure the running time

LOCAL_TZ = os.getenv("LOCAL_TZ") or "Asia/Ho_Chi_Minh"
DAILY_RUN_HOUR = int(os.getenv("DAILY_RUN_HOUR", "7"))     # 0-23  -> default 05:00
DAILY_RUN_MINUTE = int(os.getenv("DAILY_RUN_MINUTE", "0")) # 0-59
USE_DAILY_CANDLES = os.getenv("USE_DAILY_CANDLES", "1")    # "1"=force 1d candles at run, "0"=keep TIMEFRAME

def seconds_until_next_run(hour=DAILY_RUN_HOUR, minute=DAILY_RUN_MINUTE, tz_name=LOCAL_TZ):
    now = datetime.now(ZoneInfo(tz_name)).replace(second=0, microsecond=0)
...
TIMEFRAME = os.getenv("SCAN_TIMEFRAME") or '1d'  # or '1h', '1d'
...
def run_screener():
    ...
    for symbol in symbols:
        ...
        try:
            ...
            # If no signal triggered
            if not has_signal:
                print(f"No signal sir for {symbol}")
        except Exception as e:
...
if __name__ == "__main__":
    while True:
        ...
        run_at = datetime.now(ZoneInfo(LOCAL_TZ)) + timedelta(seconds=wait_seconds)
        print(f"⏳ Waiting {wait_seconds} seconds until daily run at {run_at.strftime('%Y-%m-%d %H:%M %Z')}...")
        time.sleep(wait_seconds)

        # Optionally force daily candles on run
        if USE_DAILY_CANDLES == "1":
            os.environ["SCAN_TIMEFRAME"] = "1d"

        print("▶️ Running daily scanner now...")
        try:
            run_screener()
        except Exception as e:
            print(f"🚨 Daily run failed: {e}")
        finally:
            # after run, sleep ~60s to avoid immediate re-trigger if time aligns
            time.sleep(60)
